...
const DetailNonisata = () => {
  const [nonwisata, setNonwisata] = useState(null);
  const { id } = useParams();
  const [activeSection, setActiveSection] = useState("");

  const getNonwisataById = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/nonwisata/${id}`);
              ...
              <div className="flex gap-10">
                  <div className="w-3/5 h-72 border-2 p-2 rounded-md border-[#3c87ca]">
                    <iframe
                      ...
                      src={nonwisata.link_maps}
                      frameborder="0"
                    ></iframe>
                  </div>
                <div className="w-1/3">
                <h1 className="font-body">Jam buka: {nonwisata.jam_buka} WIB.</h1>
                  <h1 className="font-body">Alamat</h1>
                    <p className="font-body text-sm">{nonwisata.alamat}</p>
                </div>
              </div>
            </div>
            <div className="">
